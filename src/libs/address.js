...
for (let key in cities) {
    ...
    for (let shiKey in cities[key]) {
        var shiObj = {
            label: shiKey,
            value: shiKey,
            children: []
        }
        for (let i = 0, len = cities[key][shiKey].length; i < len; i++) {
            var xianObj = {
                value: cities[key][shiKey][i],
                label: cities[key][shiKey][i]
            }
            shiObj.children.push(xianObj);
        }
        shengObj.children.push(shiObj);
    }
    options.push(shengObj);
}
